...
**Core idea:** Rule-based HRI is rigid. This system perceives 4 cues — **emotion, gesture, motion, context** — from video, fuses them to predict a human **intent** (F01–F10), and maps intent to a **robot action**. Finally run on a **Jetson Orin Nano**.
...
- Ubuntu HPC, **RTX 3060**, 32 GB RAM. Plenty for this: feature extraction is the only GPU-heavy step; fusion training is tiny (CPU-capable).
- Deployment target: **Jetson Orin Nano** (a different machine). Everything trained here must export to ONNX.
- Use a single conda/venv environment. Pin versions in `requirements.txt` from day one — the Jetson port depends on knowing exact versions.
...
All downstream code assumes **one probability vector per video clip per modality**:
...
| Motion   | sitting, stand, walk, run, stepping_back, leaning_forward | 6 |
...
Known inconsistencies to resolve before extraction:
- **Row 39** (kitchen, playful thumbs-down while smiling): intent F01 (greet) contradicts sub-intent S08 (frustration) and action A08 (de-escalate). Team must pick one chain.
- **Row 11** (classroom, happy + raise hand): labeled F05 "Neutral pass" but sub-intent "Volunteer an Answer" and action "Give a chance" — an active response, not a pass. Confirm intended intent.
- **Sub-intent S09 duplicated** in classroom (row 9 Emergency vs row 11 Volunteer). Renumber.
- Sweep all rows for intent/sub-intent/action chains that don't tell the same story.
...
## 7. Phase 1 — Feature extraction & baselines (Days 2–6)

### 7.1 Feature table (the key efficiency mechanism)

Run all 4 models over all ~1,950 videos **once**, save `data/features/features_v1.parquet`, one row per video:

```
video_id, scenario_id, actor_id, split, context_gt,
emotion_probs (7), gesture_probs (7), motion_probs (7), context_probs (2),
missing_flags (4 bool, from table's Missing column),
intent_label (F01–F10), sub_intent, action_label, test_cases
```
...
## 8. Phase 2 — Fusion model, augmentation, evaluation (Days 6–14)
...
2. **Cue recombination:** build synthetic samples by combining real cue vectors from different videos into new combinations; label via the table's cue→intent logic. Only generate combinations whose correct intent is unambiguous. Targets: conflict cases (T02), rare classes (F09, F10), cross-context transfer (classroom F09). Cap synthetic at ≤2–3× real.
...
- Overall intent accuracy + macro-F1 (macro-F1 matters: F09/F10 are rare) + confusion matrix, on the real test set.
...
## 9. Phase 3 — Jetson export & actions (Days 14–21)
...
- Provide a `deploy/` folder: ONNX files, preprocessing code per modality, a single `pipeline.py` (video/camera → 4 models → fusion → intent → action) that the team ports to the Jetson. Target: process a 2–4 s window in ≤1 s on Orin Nano (FP16); document expected FPS from HPC profiling as reference.
- **Stretch — actions:** `src/actions/policy.py`: dict lookup intent (+context) → action A01–A13 from `labels.csv`, with a confidence threshold: if max fusion softmax < τ (tune on val, ~0.5), fall back to F05 Neutral pass / safe action. Safety rule: **F02 Emergency must have the lowest miss rate** — check its recall explicitly; if needed, lower its decision threshold.

## 10. Week-by-week schedule

| Days | Milestone |
|---|---|
| 1–2 | Phase 0: model audit, table fixes confirmed with team, `labels.csv`, inventory, repo scaffold |
| 3–5 | Feature extraction complete (`features_v1.parquet`); trial ONNX export of all 4 models |
| 5–6 | Baselines done: rule-based, unimodal, concat-MLP + first results table |
| 7–10 | Attention fusion + modality dropout; beats baselines on val; T03 masking sweep |
| 10–12 | Augmentation (recombination, jitter) + ablations; T02 conflict results |
| 12–14 | Full T01–T05 evaluation on real test set; unimodal retraining only if error analysis demands; freeze `features_vN` |
| 15–17 | ONNX pipeline + `deploy/` package; HPC latency profile |
| 17–19 | Intent→action module + end-to-end demo script |
| 19–21 | Buffer: final results tables/figures for thesis, demo video, reproducibility check (fresh-clone run of scripts 00→04) |
...
- Dataset is frozen — no new recording.
...
2. Confirm resolutions for table rows 39 and 11, and the S09 renumbering (§5.3).
3. Exact definitions of test cases T01–T05 (the table references them; the definitions live with the team).
4. Are actor IDs recoverable per video (filenames / folder / memory)?
5. Confirm the class lists per modality (§5.2) against the actual trained model heads.
