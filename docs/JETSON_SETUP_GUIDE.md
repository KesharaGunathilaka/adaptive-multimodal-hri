...
Everything you need is already packaged in `jetson_deploy/` on this repo (see
`jetson_deploy/README.md` for the short version — this doc is the long
version, written for someone who hasn't touched a Jetson before). You'll work
directly at the Jetson's own monitor/keyboard and move files over by USB
drive, per your setup.
...
## Still open — tell me these when you have them
...
Once the board is running, next milestones per `docs/WORKLOG.md` /
`[[fusion-phase0-audit]]` context: benchmarking actual inference latency of
each of the 4 models plus the fusion layer on-device is the real target of
this exercise, not just "it runs."
