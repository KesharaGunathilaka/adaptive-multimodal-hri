...
Examples:
    # Full fused context on every video under videos/
    python modalities/context/video_test.py

    # Only the classroom clips, scene model only
    python modalities/context/video_test.py --mode scene --input videos/Classroom

    # Gaze only, and also save annotated output + logs
    python modalities/context/video_test.py --mode gaze --save

Controls (preview window): 'n' next video, space pause, 'q'/ESC quit.
"""
...
from modalities.context.scene_classification.scene_classifier import (
    SceneClassifier,
    DEFAULT_CLASSES,
)
...
from modalities.context.context_pipeline import ContextPipeline, _draw_overlay

VIDEO_EXTS = {".mp4", ".avi", ".mov", ".mkv", ".flv", ".wmv"}
KNOWN_SCENES = set(DEFAULT_CLASSES)
...
def make_processor(mode):
    ...
    if mode == "scene":
        ...
        def proc(frame, fps):
            ...
            cv2.putText(
                frame,
                f"scene: {r['label']} ({r['confidence']:.2f})  {fps:.1f} FPS",
                (15, 35),
    ...
    if mode == "object":
        ...
        def proc(frame, fps):
            ...
            cv2.putText(
                annotated,
                f"stable: {stable_txt}  ({len(detections)} det)  {fps:.1f} FPS",
            ...
            info = {
                ...
                "counts": counts,
                "stable_categories": sorted(stable),
            }
            return annotated, info, None
    ...
    if mode == "gaze":
        ...
        def proc(frame, fps):
            ...
            if g.has_face and g.gaze_point and g.face_bbox:
                ...
                gp = (int(g.gaze_point[0]), int(g.gaze_point[1]))
                cv2.arrowedLine(frame, (cx, cy), gp, (0, 255, 0), 2, tipLength=0.2)
                status = "ROBOT" if g.looking_at_robot else "AWAY"
                cv2.putText(
                    frame,
                    f"yaw {g.yaw:+.0f} pitch {g.pitch:+.0f} -> {status} eng {g.engagement:.2f}",
                    (15, 35),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.7,
                    (0, 255, 0) if g.looking_at_robot else (0, 165, 255),
                    2,
                )
            else:
                cv2.putText(
                    frame, "No face", (15, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2
                )
...
def run_video(video_path, proc, args, gt_scene, out_dir):
    ...
    if args.save:
        ...
        out_video = out_dir / f"{video_path.stem}_{args.mode}.mp4"
        writer = cv2.VideoWriter(
            str(out_video), cv2.VideoWriter_fourcc(*"mp4v"), src_fps, (width, height)
        )

    frame_idx = 0
    correct = 0
    scored = 0  # frames with a confident (non-"uncertain") scene prediction
    prev = time.time()
    fps = 0.0
    paused = False
    quit_all = False
    ...
    while True:
        if not paused:
            ...
            if gt_scene and pred_scene is not None:
                if pred_scene != "uncertain":
                    scored += 1
...
def _jsonable(info):
    """Best-effort conversion of an info dict to JSON-serializable form."""
...
def main():
    ...
    parser.add_argument(
        "--input", default=str(REPO_ROOT / "videos"), help="Video file or folder."
    )
    parser.add_argument(
        "--mode", default="context", choices=["context", "scene", "object", "gaze"]
    )
    ...
    out_root = REPO_ROOT / "runs" / "context_test" / args.mode

    totals = {"correct": 0, "scored": 0}
    try:
        for i, video in enumerate(videos, 1):
            gt = ground_truth_scene(video)  # use full path (root may be the scene folder)
