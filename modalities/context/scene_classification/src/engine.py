"""
Shared training engine: a two-stage fine-tuning recipe used by both model
comparison and full training, plus metric helpers, mixup, AMP and seeding.

Recipe:
  Stage 1 - train classifier head only (frozen backbone), CrossEntropy,
            Adam @ HEAD_LR.
  Stage 2 - unfreeze, full fine-tune with CE + label smoothing, mixup,
            gradient clipping, cosine LR with linear warmup, and automatic
            mixed precision (AMP) on CUDA.

Class weights are passed through (the scene dataset is balanced, so they are
~1.0). Selection metric defaults to accuracy.
"""
import math
import random
...
def fit_two_stage(model, train_loader, val_loader, device, class_weights,
                  ...
                  use_amp=True, select_metric="accuracy", verbose=True):
    """
