"""
Stage 1 - Compare candidate backbones with the two-stage recipe and pick the
most suitable one for deployment.

For each model in the zoo it measures: parameter count, size (MB), best
accuracy / macro-F1, GPU+CPU inference latency, and train time. The winner is
the highest **accuracy** model within the size budget.

Outputs under reports/comparison/: comparison.csv/json/png + COMPARISON_REPORT.md

Run:
    python scripts/compare_models.py --stage1_epochs 3 --stage2_epochs 8
    python scripts/compare_models.py --models EfficientNet-B0 ResNet18
"""
import argparse
import json
import os
...
from config import BATCH_SIZE, CHECKPOINT_DIR, NUM_WORKERS, REPORT_DIR, SIZE_BUDGET_MB
...
def measure_latency(model, device, n=60, warmup=10):
    """Mean inference latency (ms) for a single 224x224 image."""
...
def plot_comparison(df, path):
    ...
    axes[0, 0].bar(order["model"], order["accuracy"], color="#3b7dd8")
    axes[0, 0].set_title("Accuracy (selection metric)"); axes[0, 0].set_ylabel("%")
    axes[0, 0].tick_params(axis="x", rotation=25)
    axes[0, 1].scatter(df["size_mb"], df["accuracy"], s=60, color="#e07a3b")
    for _, r in df.iterrows():
        axes[0, 1].annotate(r["model"], (r["size_mb"], r["accuracy"]), fontsize=8)
    axes[0, 1].axvline(SIZE_BUDGET_MB, ls="--", color="red", label=f"{SIZE_BUDGET_MB:.0f} MB budget")
    axes[0, 1].set_xlabel("size (MB)"); axes[0, 1].set_ylabel("accuracy (%)")
    axes[0, 1].set_title("Accuracy vs size"); axes[0, 1].legend()
    axes[1, 0].bar(order["model"], order["gpu_ms"], color="#5aa469")
    axes[1, 0].set_title("GPU inference latency"); axes[1, 0].set_ylabel("ms / image")
    axes[1, 0].tick_params(axis="x", rotation=25)
    axes[1, 1].bar(order["model"], order["params_m"], color="#9b6dd8")
    axes[1, 1].set_title("Parameters"); axes[1, 1].set_ylabel("millions")
...
def write_report(df, winner, args, path):
    order = df.sort_values("accuracy", ascending=False)
    lines = [
        "# Scene Model - Stage 1: Model Comparison Report\n",
        "Candidate ImageNet-pretrained backbones trained with the identical "
        "two-stage recipe and ranked by **accuracy** within a "
        f"**{SIZE_BUDGET_MB:.0f} MB** deployment budget.\n",
        f"- Search length: {args.stage1_epochs} head-only + {args.stage2_epochs} full-finetune epochs/model",
        f"- Batch size: {args.batch_size}\n",
        "## Results\n",
        order.to_markdown(index=False),
        "",
        f"\n## Recommendation: **{winner}**\n",
        f"- Best accuracy among models within the {SIZE_BUDGET_MB:.0f} MB budget.\n",
        "## Next step\n",
        "```",
        f'python scripts/train.py --model "{winner}"',
        "```",
        "",
        "> Note: these numbers come from a shortened search; the final accuracy "
        "comes from the full training run (Stage 2).",
    ]
...
def main():
    ...
    winner = pool.sort_values("accuracy", ascending=False).iloc[0]["model"]
